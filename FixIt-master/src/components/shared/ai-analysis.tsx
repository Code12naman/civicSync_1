...
const AiAnalysisComponent: React.FC<AiAnalysisComponentProps> = ({ onClose }) => {
  ...
  const [analysisResult, setAnalysisResult] = useState<AnalyzeIssueImageFormFieldsOutput | null>(null);
  ...
  const handleAnalysis = async (imageDataUri: string) => {
    ...
    try {
      ...
      if (!result || typeof result !== 'object') {
        ...
        // Using dedicated form fields output; display directly.

      setAnalysisResult(safeResult); // Store sanitized result
      // Treat every valid AI output as a successful analysis.
      toast({
        title: 'Analysis Complete',
        description: `Detected: ${safeResult.issueType}, Priority: ${safeResult.priority}`,
      });
    } catch (err) {
   ...
   const handleUseDetails = () => {
        ...
        const query = new URLSearchParams({
          aiType: analysisResult.issueType,
          aiTitle: analysisResult.title,
          ...
          <CardContent className="p-4 space-y-4">
            ...
            {analysisResult && !isLoading && (
              <div className="space-y-3 text-sm border-t pt-4 mt-4">
                 ...
                 <p><strong>Detected Type:</strong> {analysisResult.issueType}</p>
                 <p><strong>Suggested Priority:</strong> {analysisResult.priority}</p>
                 <p><strong>Suggested Title:</strong> {analysisResult.title}</p>
                 <p><strong>Suggested Description:</strong> {analysisResult.description}</p>
                  {/* Button to Proceed */}
