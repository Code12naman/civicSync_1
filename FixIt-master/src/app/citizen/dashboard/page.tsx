...
export default function CitizenDashboardPage() {
  ...
  // Animated counters using Framer Motion animate -> updates local state numbers
  const totalMv = useMotionValue(0);
  const progressMv = useMotionValue(0);
  const resolvedMv = useMotionValue(0);
  const [totalDisplay, setTotalDisplay] = useState(0);
  const [progressDisplay, setProgressDisplay] = useState(0);
  const [resolvedDisplayState, setResolvedDisplayState] = useState(0);

  useEffect(() => {
    const controls = [] as any[];
    controls.push(animate(totalMv, totalReported, { duration: 0.8, ease: "easeOut", onUpdate: v => setTotalDisplay(Math.round(v)) }));
    controls.push(animate(progressMv, issuesInProgress, { duration: 0.8, ease: "easeOut", onUpdate: v => setProgressDisplay(Math.round(v)) }));
    controls.push(animate(resolvedMv, issuesResolved, { duration: 0.8, ease: "easeOut", onUpdate: v => setResolvedDisplayState(Math.round(v)) }));

    return () => controls.forEach(c => c.stop && c.stop());
  }, [totalReported, issuesInProgress, issuesResolved]);
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.section
        ...
        className="relative overflow-hidden rounded-lg p-8 md:p-10 bg-gradient-to-br from-emerald-50/80 via-white to-green-50/60"
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
          <div className="max-w-2xl space-y-3">
            ...
            <p className="text-base text-gray-600 leading-relaxed">
              Your reports help build cleaner, safer cities. Track progress or report new issues below.
            </p>

            <div className="mt-6 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold shadow-md hover:shadow-lg transition-all">
                <Link href="/citizen/dashboard/report">
                  <FilePenLine className="mr-2 h-5 w-5" />
                  Report New Issue
                </Link>
              </Button>
              <Button variant="outline" size="lg" asChild className="border-emerald-300 text-emerald-700 hover:bg-emerald-50">
          ...
          <div className="flex gap-4 items-center">
            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 bg-white rounded-lg border-t-2 border-emerald-500 shadow-sm min-w-[110px]">
                <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Total</div>
                <div className="text-3xl font-bold text-gray-900 mt-1">{totalDisplay}</div>
                <div className="text-xs text-gray-500 mt-1">Reported</div>
              </div>
              <div className="p-4 bg-white rounded-lg border-t-2 border-emerald-500 shadow-sm min-w-[110px]">
                <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Active</div>
                <div className="text-3xl font-bold text-gray-900 mt-1">{progressDisplay}</div>
                <div className="text-xs text-gray-500 mt-1">In Progress</div>
              </div>
              <div className="p-4 bg-white rounded-lg border-t-2 border-emerald-500 shadow-sm min-w-[110px]">
                <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Resolved</div>
                <div className="text-3xl font-bold text-gray-900 mt-1">{resolvedDisplayState}</div>
      ...
      <section className="space-y-6">
         ...
         <div className="grid gap-6 md:grid-cols-3">
             ...
             <SummaryCard
                 ...
                 description="Being addressed by city teams"
                 imageUrl="https://picsum.photos/seed/impact2/100/100"
                 imageHint="tools worker progress"
                 icon={<LoaderCircle className="h-6 w-6 text-emerald-600 animate-spin" />}
                 isLoading={loading}
             />
             <SummaryCard
      ...
      {/* Recent Issues Section */}
       <section id="recent-issues" className="space-y-4">
          ...
          {filteredIssues.map((issue) => (
            <motion.div key={issue.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.01 }} transition={{ duration: 0.25 }}>
