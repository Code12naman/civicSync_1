...
import type { AnalyzeIssueImageFormFieldsOutput } from '@/ai/flows/analyze-issue-image-flow';
import { useSearchParams, useRouter } from 'next/navigation';
import { getUserProfile, setUserProfile } from '@/lib/mock-users';
...
export default function ReportIssuePage() {
  ...
  const [aiAnalysisResult, setAiAnalysisResult] = useState<AnalyzeIssueImageFormFieldsOutput | null>(null);
  ...
  // Actively apply AI query param suggestions to the form after mount as well,
  // in case defaultValues are not re-applied due to navigation or hydration.
  useEffect(() => {
    if (aiType) {
      form.setValue('type', aiType);
    }
    if (aiTitle) {
      form.setValue('title', aiTitle);
    }
    if (aiDescription) {
      form.setValue('description', aiDescription);
    }
    if (aiPriority) {
      form.setValue('priority', aiPriority);
      setCurrentPriority(aiPriority);
    }
  }, [aiType, aiTitle, aiDescription, aiPriority, form]);
 ...
 useEffect(() => {
    if (typeof window !== 'undefined') {
        try {
            ...
            if (storedImage) {
                ...
                // If user navigated without AI query params, auto-run analysis on the stored image
        const hasAiParams = Boolean(aiType || aiTitle || aiDescription || aiPriority);
        if (!hasAiParams) {
  ...
  const handleAiAnalysis = async (imageDataUri: string) => {
    ...
    try {
      // Send image to backend API as multipart/form-data
      const blob = await fetch(imageDataUri).then(r => r.blob());
      const file = new File([blob], `capture-${Date.now()}.jpg`, { type: blob.type || 'image/jpeg' });
      const fd = new FormData();
      fd.append('image', file);
      fd.append('description', currentDescription || '');
      ...
      const result = (await res.json()) as AnalyzeIssueImageFormFieldsOutput;

      // Sanitize AI output to avoid empty strings or invalid enums
      const safeResult = {
        issueType: ((): AnalyzeIssueImageFormFieldsOutput['issueType'] => {
          const allowed = ['pothole','garbage','streetlight','water leakage','road damage','other'] as const;
          return allowed.includes(result.issueType as any) ? result.issueType : 'other';
        })(),
        priority: ((): IssuePriority => {
          const allowed = ['Low','Medium','High'] as const;
          return (allowed as readonly IssuePriority[]).includes(result.priority as IssuePriority) ? result.priority as IssuePriority : 'Medium';
        })(),
        title: (result.title || '').trim() || 'Reported Civic Issue',
        description: (result.description || '').trim() || 'A civic issue has been reported. Please review the image and add details if needed.',
      };

      setAiAnalysisResult(safeResult);

      // Map form issue types to app IssueType domain
      const mappedType: IssueType =
        safeResult.issueType === 'garbage' ? 'Garbage' :
        safeResult.issueType === 'streetlight' ? 'Streetlight' :
        safeResult.issueType === 'pothole' || safeResult.issueType === 'road damage' ? 'Road' :
        safeResult.issueType === 'water leakage' ? 'Other' :
        'Other';

      // Auto-fill the form; users can edit afterwards
      form.setValue('type', mappedType);
      form.setValue('title', safeResult.title);
      form.setValue('description', safeResult.description);
      form.setValue('priority', safeResult.priority);
      setCurrentPriority(safeResult.priority);

      // Treat every valid AI output as a successful analysis.
      toast({ title: 'AI Analysis Complete', description: `Suggested Type: ${safeResult.issueType}, Priority: ${safeResult.priority}` });
    } catch (err) {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        // Use the actual logged-in user's email instead of hardcoded value
        const userId = userEmail || 'citizen123';
        ...
        try {
            ...
            // Update user profile to increment reports submitted
            if (userEmail) {
                const profile = getUserProfile(userEmail);
                if (profile) {
                    profile.reportsSubmitted = (profile.reportsSubmitted || 0) + 1;
              ...
              {aiAnalysisResult && (
                    <Alert variant="default" className="text-sm bg-accent/10 border-accent/30">
                      ...
                      <AlertTitle>AI Analysis Suggestions</AlertTitle>
                       <AlertDescription>
                         Type: {aiAnalysisResult.issueType}, Priority: {aiAnalysisResult.priority}, Title: "{aiAnalysisResult.title}". Auto-filled; you can edit any field.
                      </AlertDescription>
                  </Alert>
               )}
